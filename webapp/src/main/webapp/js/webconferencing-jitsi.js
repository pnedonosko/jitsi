/**
 * Jitsi provider module for Web Conferencing. This script will be used to add a provider to Web Conferencing module and then
...
(function($, webConferencing) {
	...
	if (webConferencing) {
					...
					if (currentUserIMID) {
						context.details().done(function(target) {
							...
							if (ims.length > 1) {
								...
								// If not users compatible with Jitsi IM type found, we reject, thus don't show the button for this context
	    ...
	    this.showSettings = function() {
			...
			/**
			 * Set connector settings from the server-side. Will be called by script of JitsiPortlet class.
			 */
		...
		window.console && window.console.log("WARN: webConferencing not given and eXo.webConferencing not defined. Jitsi provider registration skipped.");
	}
